...
void RenderWithShader(TGAImage& image, Model& model, 
    ...
    for (int i = 0; i < model.nfaces(); i++) {
        ...
        for (int j = 0; j < 3; j++) {
            float w = clip_coords[j].w;
            if (w == 0.0f) w = 1.0f;
            
            screen_coords[j] = vec3(
                (clip_coords[j].x / w + 1.0f) * width / 2.0f,
                (1.0f - clip_coords[j].y / w) * height / 2.0f,
                clip_coords[j].z / w
            );
        }
        
        rasterize_triangle_with_shader(screen_coords, zbuffer, image, 
                                       shader, width, height);
    }
}
...
void rasterize_triangle_with_shader(
    ...
    for (int i = 0; i < 3; i++) {
        ...
        bboxmax.x = std::min((float)(width - 1), std::max(bboxmax.x, pts[i].x));
        bboxmax.y = std::min((float)(height - 1), std::max(bboxmax.y, pts[i].y));
    }
